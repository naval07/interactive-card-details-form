...
cardHolderNameInput.onkeyup = function() {
    document.getElementById("on-card-cardholder-name").innerHTML = cardHolderNameInput.value.toUpperCase();
...
cardExpMonthInput.onkeyup = function() {
    document.getElementById("on-card-expiration-date").innerHTML = cardExpMonthInput.value + "/" + cardExpYearInput.value
    // Reset style in case of previous error
...
cardNumInput.onkeydown = function(event) {
    ...
    if (cardNumInput.value.match(/[0-9]/g)) {
        ...
        if (numLength % 4 == 0 && event.key !== 'Backspace' && numLength < 16){
            ...
            if (fourNumberList.length >= 1 && cardNumInput.value[cardNumInput.value.length - 1] !== " "){
                cardNumInput.value += " ";
            }
        }
    }
...
function validateMonth() {
    ...
    } else if (Number(cardExpMonthInput.value) < 1 || Number(cardExpMonthInput.value) > 12) {
